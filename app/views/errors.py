...
@html_view(template='errors/404.tpl', response_class=HttpResponseNotFound, subtitle='Oops!')
def handler404(request):
    logger.warning('HTTP 404 - %s - %s - "%s"' % (request.get_full_path(), request.META.get('REMOTE_ADDR'), request.META.get('HTTP_USER_AGENT')))


@html_view(template='errors/500.tpl', response_class=HttpResponseServerError, subtitle='Oops!')
